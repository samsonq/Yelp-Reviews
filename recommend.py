...
def group_by_centroid(restaurants, centroids):
    ...
    """
    centroid = lambda x: find_closest(restaurant_location(x), centroids)
    lst = [[centroid(r), r] for r in restaurants]
...
def best_predictor(user, restaurants, feature_fns):
    ...
    predictors = [find_predictor(user, reviewed, func) for func in feature_fns]
    feature = max(predictors, key=lambda x: x[1])[0]
    return feature
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        ...
        if r in reviewed:
            all_restaurants[name] = user_rating(user, name)
        else:
            all_restaurants[name] = predictor(r)
